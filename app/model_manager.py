...
class ModelManager:
    """Manages multiple models with fallback support for Election Assistant"""
    ...
    def __init__(self):
        ...
        self.app_name = os.getenv('OPENROUTER_APP_NAME', 'ElectionAssistant')
        ...
        self.agent_models = {
            'process_explainer': os.getenv('PROCESS_EXPLAINER_MODEL', self.primary_model),
            'timeline_agent': os.getenv('TIMELINE_AGENT_MODEL', self.primary_model),
            'step_guide_agent': os.getenv('STEP_GUIDE_MODEL', self.primary_model),
            'faq_agent': os.getenv('FAQ_MODEL', self.primary_model),
            'accessibility_agent': os.getenv('ACCESSIBILITY_MODEL', self.primary_model)
        }
        ...
        logger.info(f"🚀 Model Manager initialized for Election Assistant")
    ...
    def _get_agent_temperature(self, agent_type: str) -> float:
        temperatures = {
            'process_explainer': 0.3,
            'timeline_agent': 0.2,
            'step_guide_agent': 0.4,
            'faq_agent': 0.3,
            'accessibility_agent': 0.4
        }
